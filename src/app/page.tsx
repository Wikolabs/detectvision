...
export default function DemoPage() {
  ...
  async function generate() {
    ...
    setLoading(true);
    try {
      const r = await fetch("/offers/detectvision/demo/api/demo", {
        method: "POST",
    ...
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .wk-btn-primary { background: ${PAL.accent}; color: #04141A; border: none; border-radius: 6px; padding: 13px 22px; font-weight: 700; font-size: 14px; cursor: pointer; font-family: inherit; transition: opacity .2s, transform .2s; display: inline-flex; align-items: center; gap: 8px; }
        ...
        .file-picker-label { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 14px; border: 1px dashed ${PAL.border}; border-radius: 6px; background: ${PAL.surface}; cursor: pointer; font-size: 13px; color: ${PAL.txt2}; transition: border-color .2s, background .2s; }
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 6, padding: 22 }}>
            <h2 style={{ fontSize: "0.92rem", color: PAL.txt2, fontWeight: 700, margin: "0 0 14px" }}>{t.fileLabel}</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
              ...
              <label style={{ fontSize: 11, color: PAL.txt3, fontWeight: 700, marginTop: 6 }}>{t.skuLabel}</label>
              <input className="wk-input" value={sku} onChange={(e) => setSku(e.target.value)} placeholder={t.skuPlaceholder} />
              <label style={{ fontSize: 11, color: PAL.txt3, fontWeight: 700, marginTop: 6 }}>{t.sceneLabel}</label>
          ...
          <section style={{ background: PAL.bg2, border: `1px solid ${PAL.border}`, borderRadius: 6, padding: 22, minHeight: 420, display: "flex", flexDirection: "column" }}>
            ...
            {brief && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 18, paddingTop: 18, borderTop: `1px solid ${PAL.border}` }}>
                ...
                <button className="wk-btn-ghost" onClick={() => showToast(t.auditMock)}>{t.saveAudit}</button>
              </div>
            )}
